...
def registros_api_completo(request):
    ...
    # Luego la lógica de DataTables (filtros por columna, paginación, etc.)
    ...
...
class FUIDUpdateView(LoginRequiredMixin, UpdateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        # Verificar si es superusuario o si pertenece a la misma oficina
...
def detalle_fuid(request, pk):
    ...
    # Verificar que sea superusuario o pertenezca a la misma oficina
    if not request.user.is_superuser and fuid.oficina_productora != request.user.perfil.oficina:
...
def editar_registro_de_fuid(request, fuid_id, registro_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            return redirect('detalle_fuid', pk=fuid_id)  # O la vista de detalle que prefieras
        else:
            for field, errors in form.errors.items():
                field_name = form.fields[field].label
                for error in errors:
